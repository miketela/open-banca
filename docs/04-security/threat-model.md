...
| T15 | LLM gateway | Information Disclosure | LiteLLM proxy comprometido captura prompts | med | Prompts no contienen creds (T02); URLs API keys whitelisted; opción on-prem LiteLLM; PII de usuario redactada pre-envío ([ADR-0020](../adr/0020-pii-redact-llm-boundary.md)) | Post-redacción: proveedor LLM ve orden de magnitud de saldo y DOM sin nombre/cuenta; gap en onboarding run — ver T18 |
...
| T18 | A3 / PII usuario en banco | Information Disclosure | Post-login el banco renderiza nombre, números de cuenta y saldos en DOM + screenshots; todo este payload se envía al LLM provider (Anthropic US, DeepSeek CN) sin redacción | high | Pre-send PII filter middleware (Capa 1 texto/DOM + Capa 2 imagen) actúa antes de LiteLLM routing; orden de magnitud de saldo preservado, valor exacto redactado ([ADR-0020](../adr/0020-pii-redact-llm-boundary.md)) | Gap en primer run (onboarding): Capa 2 imagen no puede actuar sin `pii_regions` en el map |
| T19 | A8 master passphrase | Information Disclosure | OS swap o coredump expone la passphrase en memoria del proceso Python que abre el vault sqlcipher | crit | Llamar `mlock()` sobre el buffer de passphrase tras leerla del env; deshabilitar core dumps en el proceso (`resource.setrlimit(CORE, (0, 0))`); documentado en runbook de hardening | Kernel puede ignorar mlock bajo memory pressure; solución robusta requiere secret-zero via hardware HSM (postergado v2) |
| T20 | Supply chain browser-use | Tampering | Compromiso de la librería `browser-use` (path dep local o PyPI) introduce código malicioso en el agent loop | high | Path dep al directorio hermano bajo control del operador (no PyPI); hash-pinning en `uv.lock`; SBOM generado en CI; revisión de diff obligatoria en actualizaciones | Fork privado puede divergir de upstream sin revisión de seguridad |
| T21 | DSL parser / Excel | Denial of Service | Expresión regex maliciosa en un campo `pattern` del `parser.json` de un banco provoca ReDoS en el parser engine | med | Todos los regex en `parser.json` pasan por `re2` (backtrack-free); CI test con string adversarial de longitud creciente; timeout global de 500 ms por expresión | `re2` no cubre toda la sintaxis de `re`; migración de legado puede introducir sintaxis no soportada |
| T22 | A5 OTP en logs | Information Disclosure | Código OTP de Clave Móvil aparece en Langfuse trace o en OTel span durante la fase `pause_for_otp` | crit | OTP nunca se pasa como argumento de herramienta al LLM; se inyecta vía endpoint separado `/jobs/{id}/otp` y se consume en memoria; filter middleware redacta patrones `\b\d{6}\b` en contexto OTP | El endpoint OTP es un vector independiente — ver T08 |
| T23 | A2 API keys en config | Information Disclosure | API key de Anthropic/DeepSeek/Langfuse escrita en texto plano en un archivo de config versionado o en variable de entorno no cifrada | high | `.env` excluido de git (`.gitignore` forzado); CI lint verifica ausencia de patrones `sk-ant-`, `dsk-`, `ls-o-` en diffs; instrucción en runbook: file mode 0600, montado como secret Docker en producción | Responsabilidad final del operador; lint es best-effort |
| T24 | Webhook `/time` endpoint | Information Disclosure | Un endpoint `/time` o similar que devuelva el reloj del servidor permite a un atacante alinear ventanas HMAC y realizar ataques de oracle temporal para forjar o reproducir webhooks | med | No exponer endpoint `/time` en la API; el cliente debe usar el header `Date` de la respuesta o su propio reloj con tolerancia de ±5 min para validar el timestamp del webhook ([ADR-0011](../adr/0011-webhook-events-hmac.md)) | Si el cliente tiene skew >5 min rechazará webhooks legítimos — documentado en `webhooks.md` |
| T25 | A4 GitHub OIDC + cosign | Tampering | Compromiso del workflow de GitHub Actions que firma mapas community permite generar firmas cosign válidas para un `map.json` malicioso | high | OIDC token es efímero (TTL minutos); policy de firma requiere claim `repository` + `ref` específicos verificados en runtime; repo protegido con branch protection + required reviews; alertas en GitHub Advanced Security | Si el repo GitHub está comprometido (admin token), toda la cadena cae |
| T26 | Ley 81 PA Art. 13 transferencia transfronteriza | Information Disclosure (Compliance) | Datos personales del titular de cuenta (nombre, número de cuenta, saldo) transferidos a LLM provider en jurisdicción extranjera (US/CN) sin base habilitante de Ley 81 Art. 13 | high | Pre-send PII filter (ADR-0020) reduce el payload a datos anonimizados/seudonimizados antes de la transferencia; si el filter falla, la transferencia ocurre sin consentimiento explícito del titular | Consentimiento del titular no está capturado en v1; el operador asume responsabilidad contractual; v2 deberá formalizar base habilitante o migrar a LLM self-hosted |
| T27a | A1/A6 endpoint `POST /jobs/{id}/human-input` (ADR-0021) | Tampering / Injection | Cliente HTTP malicioso (o cliente legítimo comprometido) envía `answer` con XSS, control chars, embed direction marks, o payload de >256 bytes; valor termina ejecutándose en el browser del banco o corrompe la cache | high | Input validation: length cap 256 B UTF-8; whitelist Unicode categories (rechaza Cc, Cf); rate limit 3 intentos por `(job_id, field_key)`; audit log no renderiza `answer` en claro; runner usa Playwright `page.fill` (no JS eval) | XSS reflejado en el banco fuera de scope (responsabilidad del banco); operador hostil con token API puede inyectar pero ya tiene credenciales |
| T27b | `security_q` cache stale → account lockout | Denial of Service | Respuesta cacheada queda obsoleta (banco rotó preguntas, cache poisoning previo, intervención del usuario en banco). Reuso silencioso del valor wrong en próximos jobs causa fallos repetidos de login → banco bloquea la cuenta del titular | high | TTL 90 días por entrada; **invalidación dura inmediata** si el step posterior a `prompt_user` con cache hit dispara `assertion_failed`/`http_error` (workflow borra `cache_key` antes de propagar fallo); cap 3 attempts por `field_key` por sesión; circuit breaker existente (T04) cubre el fallback final | Si el banco no devuelve `assertion_failed` claramente (e.g. silent rejection con dashboard parcial), la invalidación no se dispara → un job más antes del próximo recall |
...
| Anthropic/DeepSeek no exfiltran prompts a terceros | Términos contractuales de los proveedores | PII del usuario (nombre, cuenta, saldo exacto) es redactada antes de enviar al provider ([ADR-0020](../adr/0020-pii-redact-llm-boundary.md)); residual: orden de magnitud de saldo y DOM estructural; self-host LLM en v2 eliminaría el residual |
...
- **T05 / T25**: sin sigstore aún implementado, los maps community y la cadena cosign son la superficie más vulnerable.
- **T18 / T26**: el PII filter (ADR-0020) mitiga la transferencia transfronteriza, pero el gap de onboarding (primer Mapper run sin `pii_regions`) deja una ventana. Requiere documentación explícita al operador y CI canary test con señales `PII_CANARY_*`.
- **T19**: mlock sobre passphrase no implementado en v1; coredump deshabilitado en runbook pero no forzado por código. Añadir `resource.setrlimit` en el arranque del vault.
